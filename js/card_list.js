...
window.onload = async () => {
    ...
    try {
      ...
      // アプリのバージョンが変わったら（カードデータ更新含む）キャッシュを無効化する。
      // これが無いと、バージョン更新後も最大24時間ローカルの古いカードデータが表示される。
      const appVer = self.APP_VERSION || '';
      const cachedDataVersion = localStorage.getItem('dataVersion');
      const versionMatches = cachedDataVersion === appVer;
