...
>   （ダウン時トリガー含む）も実装済み
> - **効果テキストコンパイラ実装済み**（core/effects/text-compiler.js）。手書き定義の無い
>   カードを定型パターンから自動実装。発売日順に実装を進行中で、最初の製品
>   （ときのそら＆AZKi / ブルーミングレディアンス, 2024-09-20）は実用カバレッジに到達。
>   全体カバレッジは tests/coverage.html で計測（手書き+自動で約345/874種）
> - 未実装: AI Phase2（デッキ推測・先読み）、ギフト、一般の自動能力待機キュー (10.6.3)、
>   トリガー型◆能力・タイミング推しスキル、置換効果、制限カードのデッキ検証、v2のSWプリキャッシュ化
