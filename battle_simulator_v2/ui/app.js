...
async function startGame(opts = {}) {
  ...
  try {
    ...
    gameStartAnnounced = false; lastTurnKey = null; // 対戦開始の先攻/後攻アナウンスを新規対局ごとに出し直す
...
let gameStartAnnounced = false; // 対戦開始時の先攻/後攻アナウンスを1回だけ出すフラグ

/** 「あなた」に当たるプレイヤー番号（オンライン対戦者＝自分／ソロ対CPU＝人間1人）。無ければnull（ホットシート/観戦）。 */
function selfPlayerIdx() {
...
/**
 * オンライン対戦者（観戦者でない）から見て、相手の決定（=自分のものでない pending）かどうか。
 * 真なら、その効果選択・カード選択・エール送り等を自分の画面に出さない＝相手の手札/デッキの中身が漏れない。
...
function notifyTurnChange(s) {
  ...
  // 対戦開始（最初のターン）は「先攻/後攻」を視点付きで大きく告知する（誰が先攻か分かりやすく）。
  if (!gameStartAnnounced && s.turn === 1 && s.firstPlayer != null) {
    gameStartAnnounced = true;
    const self = selfPlayerIdx();
    const fpName = s.players[s.firstPlayer].name;
    toast.textContent = self != null
      ? `🎲 あなたは【${s.firstPlayer === self ? '先攻' : '後攻'}】 ─ ${fpName} のターンから開始`
      : `🎲 先攻は ${fpName} ─ ${fpName} のターンから開始`;
    toast.classList.add('start-banner');
  } else {
...
function render() {
  ...
  // 下の手札が誰のものかを明示（ホットシートの切替に気づける）＋先攻/後攻を常時表示（対局中いつでも確認できる）。
  const ownerTag = document.getElementById('hand-owner');
  const fpTag = s.firstPlayer != null ? `　<span class="fp-badge">${s.firstPlayer === handPlayer ? '先攻' : '後攻'}</span>` : '';
  ownerTag.innerHTML = `🖐 ${escapeText(s.players[handPlayer].name)} の手札（${s.players[handPlayer].hand.length}枚）${fpTag}`;
...
function renderEffectChoiceModal(s) {
  ...
  // オンライン: 相手の決定（効果選択・カード選択・エール送り等）は自分の画面に出さない。
  // → 相手のデッキサーチ候補や手札の中身が見えたり、選べてしまう問題を防ぐ（観戦者は全部見える）。
  if (hideOpponentChoice(s)) { modal.classList.remove('active'); bar.classList.remove('active'); return; }
