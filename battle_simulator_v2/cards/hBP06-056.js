 ...
 *     指定スキル「人生リセットボタン」は推し〈沙花叉クロヱ〉(hBP02-004) のSP。
 *     spOshiSkillUsedInfo.oshiNumber が hBP02-004 であることを条件にする
 *     （hBP06-044 と同じ運用。別の推しのSPスキルでは成立しない）。
 *     このホロメンはアーツが1つだけなので、軽減はそのまま「沙花叉と青春しよ？」への軽減と一致する。
 *
 * アーツ「沙花叉と青春しよ？」(120+):
...
export default {
  ...
  // 自分(src=target=このホロメン)がステージにいる間、SP推しスキル「人生リセットボタン」(推し hBP02-004)
  // 使用済みなら自身のアーツ必要無色-1
  artsCostReduceAura(src, target, engine) {
    ...
    const info = engine.state.players[ownerIdx].spOshiSkillUsedInfo;
    if (!info || info.oshiNumber !== 'hBP02-004') return [];
