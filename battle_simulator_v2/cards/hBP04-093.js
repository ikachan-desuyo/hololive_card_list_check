/**
 * ホロライブ2期生 (hBP04-093) サポート・イベント・LIMITED
 * このカードは、自分の手札がこのカードを含まずに6枚以下でなければ使えない。
 * 自分のデッキの上から4枚を見る。その中から、#2期生を持つホロメンを好きな枚数公開し、
...
export default {
  ...
  support: {
    ...
    *run(ctx) {
      ...
      const pool = [...looked];
      const candidates = pool.filter((c) => c.kind === 'holomen' && ctx.hasTag(c, '2期生'));
      const picked = yield ctx.chooseCards({
        cards: candidates,
        min: 0,
        title: '手札に加える #2期生 のホロメンを選択（任意・好きな枚数）',
        displayCards: pool,
      });
