/**
 * ホロライブ4期生（サポート・イベント・LIMITED）
 * このカードは、自分の手札がこのカードを含まずに6枚以下でなければ使えない。
 * 自分のデッキの上から4枚を見る。その中から、#4期生を持つホロメンを好きな枚数公開し、
...
export default {
  ...
  support: {
    canUse(ctx) {
      // このカードを含まずに手札6枚以下 = （このカードを除いた手札）が6枚以下
      return ctx.player.hand.length - 1 <= 6;
    },
    *run(ctx) {
      const looked = ctx.lookTopDeck(4);
      const pool = [...looked];
      const candidates = pool.filter((c) => c.kind === 'holomen' && ctx.hasTag(c, '4期生'));
      const picked = yield ctx.chooseCards({
        cards: candidates,
        min: 0,
        title: '手札に加える #4期生 のホロメンを選択（任意・好きな枚数）',
