...
export default {
  ...
  arts: {
    ...
    'わためと海辺デート': {
      *run(ctx) {
        if (ctx.player.oshi?.name !== '角巻わため') return;
        const targets = ctx.holomems('self', (e) => e.top.name === '角巻わため');
        if (targets.length === 0) return;
        const entry = yield ctx.chooseHolomem({
          side: 'self', filter: (e) => e.top.name === '角巻わため',
          title: 'エールデッキの上から1枚を送る〈角巻わため〉を選択',
        });
