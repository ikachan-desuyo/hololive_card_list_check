 ...
 *   自分のデッキを1枚引く。
 *   → 【保留】「アーツを使った時」に発火する onArtsUse トリガー機構がエンジンに無い
 *     （規約の保留機構: アーツを使った時(onArtsUse)トリガー）。装着カードをソースと
 *     するアーツ使用時トリガーのディスパッチが追加されたら、付け先が〈鷹嶺ルイ〉
 *     かつセンターポジションでアーツを使ったことを条件に ctx.draw(1) を実装すること。
 *
