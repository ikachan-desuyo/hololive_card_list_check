 ...
 *   → oshiSkill（能動）。自分の黄ホロメン（engine._hasColor: 多色・全色扱い対応）を1人選び、
...
export default {
  ...
  oshiSkill: {
    ...
    canUse(engine, ownerIdx) {
      ...
      return engine._stageHolomems(p).some((h) => h.stack[0] && engine._hasColor(h, '黄'));
    },
    *run(ctx) {
      const entry = yield ctx.chooseHolomem({
        side: 'self',
        filter: (e) => e.top && ctx.engine._hasColor(e.holomem, '黄'),
