 ...
 *   - 「1～2枚」= 非公開領域（デッキ）のサーチなので「見つからなかったことにする」を選べる
 *     （総合ルール 4.1.2.3。最低0枚も可。候補が無ければ何も出ない）。
...
export default {
  ...
  support: {
    *run(ctx) {
      let placed = 0;
      // 非公開領域（デッキ）のサーチ=「見つからなかったことにする」を常に選べる（総合ルール 4.1.2.3）。
      // 1枚目・2枚目とも任意。デッキから対象Debutを公開してステージに出す
      for (let i = 0; i < 2; i++) {
