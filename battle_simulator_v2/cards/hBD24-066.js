/**
 * カエラ・コヴァルスキア (hBD24-066) 推しホロメン・赤
 *
 * 推しスキル「レッドエンハンス」[ホロパワー：-2][ターンに1回]:
 *   このターンの間、自分の赤ホロメン1人のアーツ+20。
 *   → oshiSkill（能動）。自分の赤ホロメン（engine._hasColor: 多色・全色扱い対応）を1人選び、
 ...
 *     ※コスト[ホロパワー：-2]はエンジン側が処理するため run には書かない。
 *
 * 保留: なし（全効果実装済み）。同型カード hBD24-060（黄）に準拠。
 */
export default {
  number: 'hBD24-066',
  ...
  oshiSkill: {
    ...
    canUse(engine, ownerIdx) {
      ...
      return engine._stageHolomems(p).some((h) => h.stack[0] && engine._hasColor(h, '赤'));
    },
    *run(ctx) {
      const entry = yield ctx.chooseHolomem({
        side: 'self',
        filter: (e) => e.top && ctx.engine._hasColor(e.holomem, '赤'),
