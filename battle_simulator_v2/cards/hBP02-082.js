/**
 * ホロライブゲーマーズ（サポート・イベント・LIMITED）
 * このカードは、自分の手札がこのカードを含まずに6枚以下でなければ使えない。
 * 自分のデッキの上から4枚を見る。その中から、#ゲーマーズを持つホロメンを好きな枚数公開し、
...
export default {
  ...
  support: {
    ...
    *run(ctx) {
      ...
      const pool = [...looked];
      const candidates = pool.filter((c) => c.kind === 'holomen' && ctx.hasTag(c, 'ゲーマーズ'));
      const picked = yield ctx.chooseCards({
        cards: candidates,
        min: 0,
        title: '手札に加える #ゲーマーズ のホロメンを選択（任意・好きな枚数）',
