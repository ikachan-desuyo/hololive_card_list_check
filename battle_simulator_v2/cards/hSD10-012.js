/**
 * FLOW GLOW (hSD10-012) サポート・イベント・LIMITED
 *
 * [サポート効果] このカードは、自分の手札がこのカードを含まずに6枚以下でなければ使えない。
 *   → 使用条件: 手札（このカードを除く）が 6 枚以下のときのみ使える。
 *
 * 効果: 自分のデッキの上から4枚を見る。その中から、#FLOW GLOW を持つホロメンを好きな枚数公開し、
 *   公開したホロメンを手札に加える。そして残ったカードを好きな順でデッキの下に戻す。
 *   ※「#FLOW GLOW を持つ」= タグ「FLOW」と「GLOW」を両方持つホロメン
 *     （card_data 上では FLOW / GLOW が別々のタグとして格納されている）。
 *   ※「好きな枚数」=0 枚も可。
 *
...
export default {
  ...
  ai: {
    // 手札を増やしつつ #FLOW GLOW を探せるドロー系。盤面に FLOW GLOW がいるほど価値が高い
    supportValue({ engine, player }) {
      const hasFlowGlow = engine._stageHolomems(player).some((h) => {
        const tags = h.stack[0].tags || [];
        return tags.includes('FLOW') && tags.includes('GLOW');
      });
      return hasFlowGlow ? 26 : 14;
    },
  },
};
