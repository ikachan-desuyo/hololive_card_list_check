/**
 * Justice（サポート・イベント・LIMITED）
 * このカードは、自分の手札がこのカードを含まずに6枚以下でなければ使えない。
 * 自分のデッキの上から4枚を見る。その中から、#Justiceを持つホロメンを好きな枚数公開し、
...
export default {
  ...
  support: {
    ...
    *run(ctx) {
      ...
      const pool = [...looked];
      // #Justice のホロメンを好きな枚数（0枚可）一度に選んで手札に加える
      const candidates = pool.filter((c) => c.kind === 'holomen' && ctx.hasTag(c, 'Justice'));
      const picked = yield ctx.chooseCards({
        cards: candidates,
        min: 0,
        title: '手札に加える #Justice のホロメンを選択（任意・好きな枚数）',
