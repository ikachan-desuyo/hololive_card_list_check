/**
 * 大神ミオ（推しホロメン hBD24-064）・緑 / ライフ5
 *
 * 推しスキル「グリーンエンハンス」[ホロパワー：-2][ターンに1回]:
 *   このターンの間、自分の緑ホロメン1人のアーツ+20。
 *   → oshiSkill（能動）。自分の緑ホロメン（トップカードの色が緑）を1人選び、
...
export default {
  ...
  spOshiSkill: {
    ...
    canUse(engine, ownerIdx) {
      ...
      const p = engine.state.players[ownerIdx];
      return p.deck.some((c) => c && c.kind === 'holomen' && c.color === '緑');
    },
    *run(ctx) {
      const greens = ctx.deckCards((c) => c && c.kind === 'holomen' && c.color === '緑');
