/**
 * 星街すいせい（推しホロメン hBD24-034・青・ライフ5）
 *
 * 推しスキル「ブルーエンハンス」[ホロパワー：-2][ターンに1回]:
 *   このターンの間、自分の青ホロメン1人のアーツ+20。
 *   → oshiSkill（メイン起動型・能動）。自分の青ホロメン（トップカードの色が青）1人を選び、
 ...
 *   → spOshiSkill（能動）。デッキ（非公開領域）内の青ホロメンを1枚選び公開して手札へ。
...
export default {
  ...
  oshiSkill: {
    ...
    *run(ctx) {
      ...
      ctx.addTurnModifier({
        ...
        match: (h) => h === chosen,
        description: `このターンの間、${chosen.stack[0].name}（青）のアーツ+20`,
      });
    },
  },
  ...
  spOshiSkill: {
    ...
    *run(ctx) {
      const blues = ctx.deckCards((c) => c && c.kind === 'holomen' && c.color === '青');
