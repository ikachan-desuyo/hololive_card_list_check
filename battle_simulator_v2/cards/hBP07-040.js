 ...
 *     ctx.returnHolomemToDeck で戻す（エール／サポートはアーカイブへ、本体スタックはデッキの下へ。
 *     「デッキに戻った」記録・onReturnedToDeck 誘発込み。4.4.7 相当）。
  ...
  collabEffect: {
    ...
    *run(ctx) {
      ...
      const back = yield ctx.chooseHolomem({
        side: 'self',
        filter: (e) =>
          e.pos.zone === 'back' && ctx.nameIs(e.top, '赤井はあと') && e.top.bloomLevel === 'Debut',
        title: 'デッキの下に戻すバックの Debut〈赤井はあと〉を選択（戻すと効果発動）',
        optional: true,
      });
      if (!back) return; // コストを支払わない／対象不在なら効果なし

      // 専用プリミティブでデッキの下へ（holomemReturnedToDeckThisTurn 記録＋
      // onReturnedToDeck 誘発〔hBP07-039 / hBP07-042 連動〕を含む）
