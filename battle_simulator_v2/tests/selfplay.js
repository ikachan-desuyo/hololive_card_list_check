...
function runGame(lib, dmA, dmB, registry, seed, turns) {
  ...
  const m = {
    atk: [0, 0], atkTurns: [0, 0], noAtkLate: [0, 0], strandFuel: [0, 0],
...
export async function runSelfplay() {
  ...
  const agg = { wins: [0, 0], draws: 0, deckOut: 0, turnSum: 0,
    atk: [0, 0], atkTurns: [0, 0], noAtkLate: [0, 0], strandFuel: [0, 0], passedAtk: [0, 0], baton: [0, 0], maxCheers: [0, 0], lifeTaken: [0, 0] };
  for (const seed of seeds) {
    ...
    agg.turnSum += g.turn;
    for (let pl = 0; pl < 2; pl++) for (const k of ['atk', 'atkTurns', 'noAtkLate', 'strandFuel', 'passedAtk', 'baton', 'lifeTaken']) agg[k][pl] += m[k][pl];
    for (let pl = 0; pl < 2; pl++) agg.maxCheers[pl] = Math.max(agg.maxCheers[pl], m.maxCheers[pl]);
    const fmt = (pl) => `atk=${m.atk[pl]} atkT=${m.atkTurns[pl]} noAtkLate=${m.noAtkLate[pl]} strand=${m.strandFuel[pl]} passed=${m.passedAtk[pl]} baton=${m.baton[pl]} maxCh=${m.maxCheers[pl]} life=${m.lifeTaken[pl]}`;
  ...
  console.log(`SP-AGG| A: atk=${r1(agg.atk[0])} atkTurns=${r1(agg.atkTurns[0])} noAtkLate=${r1(agg.noAtkLate[0])} strand=${r1(agg.strandFuel[0])} passed=${r1(agg.passedAtk[0])} baton=${r1(agg.baton[0])} maxCheers=${agg.maxCheers[0]} lifeTaken=${r1(agg.lifeTaken[0])}`);
  console.log(`SP-AGG| B: atk=${r1(agg.atk[1])} atkTurns=${r1(agg.atkTurns[1])} noAtkLate=${r1(agg.noAtkLate[1])} strand=${r1(agg.strandFuel[1])} passed=${r1(agg.passedAtk[1])} baton=${r1(agg.baton[1])} maxCheers=${agg.maxCheers[1]} lifeTaken=${r1(agg.lifeTaken[1])}`);
